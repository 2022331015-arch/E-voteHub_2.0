...
const EventListItem = ({ event, onClick }) => {
  ...
  return (
    ...
    >
      ...
      {/* In event header/details, display Place for on-campus events */}
      {event?.votingMode === 'onCampus' && event?.place && (
        <div className={`text-xs ${TEXT_SECONDARY} mt-1`}><span className="font-semibold">Place:</span> {event.place}</div>
      )}
    </button>
  );
};
